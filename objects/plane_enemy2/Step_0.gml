...
// Lead Fire
_LX = player_falcon.x + (_LA * lengthdir_x(player_falcon.speed * sqrt(point_distance(x,y,player_falcon.x,player_falcon.y)), player_falcon.image_angle));
_LY = player_falcon.y + (_LA * lengthdir_y(player_falcon.speed * sqrt(point_distance(x,y,player_falcon.x,player_falcon.y)), player_falcon.image_angle));
...
// Turn Rate
turn_range = random_range(1.5,2.5)
...
if abs(difference) <= 2 && enemy_bullet_cooldown && distance_to_object(player_falcon) <= 1000
	{
	var bullet_inst = instance_create_layer (x, y,"enemy_bullets", bullet_missile_enemy);
